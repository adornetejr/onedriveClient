...
class GraphAuthenticator:
    ...
    def __init__(self):
        ...
        self.auth_provider = onedrivesdk.AuthProvider(
            ...
            auth_server_url=self.AUTORIZE_URL,
            auth_token_url=self.ACCESS_TOKEN_URL,
            scopes=self.SCOPES,
        )
    ...
    def get_auth_url(self):
        return self.auth_provider.get_auth_url(self.REDIRECT_URL)
    ...
    def get_profile(self):
        """Get a instance of OneDriveAccount."""
        # Drivers information.
        url = self.BASE_URL + '/drive/root'
        ...
        if response.status_code != requests.codes.ok:
            raise ValueError(
                'Failed to read user profile: {}'.format(data['error']['message']))

        # Authentication information.
        data['access_token'] = self.auth_provider._session.access_token
        data['refresh_token'] = self.auth_provider._session.refresh_token
